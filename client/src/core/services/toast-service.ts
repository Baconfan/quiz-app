...
export class ToastService {
  ...
  private createToastElement(message: string, alertClass: string, duration = 5000, route?: string ){
  ...
  success(message: string, duration?: number, route?: string) {
    this.createToastElement(message, 'alert-success', duration, route);
  }

  error(message: string, duration?: number, route?: string) {
    this.createToastElement(message, 'alert-error', duration, route);
  }

  warning(message: string, duration?: number, route?: string) {
    this.createToastElement(message, 'alert-warning', duration, route);
  }

  info(message: string, duration?: number, route?: string) {
